...
Vue.component("logo-recognition-viz", {
  ...
  computed: {
    logo_tracks: function () {
      ...
      for (
        ...
        if (
          "logo_recognition_annotations" in
          this.json_data.annotation_results[index]
        )
          return this.json_data.annotation_results[index]
  ...
  template: `
    <div calss="object-tracking-container">
        ...
        <div class="confidence max-w-md mx-auto bg-white/10 backdrop-blur-md p-6 rounded-xl shadow-lg flex flex-col items-center space-y-3 text-white">
  
  <span class="text-blue-300 text-base font-semibold">
  ...
  <span class="confidence-value text-blue-400 text-sm font-bold tracking-wide">
...
class Logo_Frame {
  constructor(json_data, video_height, video_width) {
    ...
    this.box = {
      ...
      width:
        ((json_data.normalized_bounding_box.right || 0) -
          (json_data.normalized_bounding_box.left || 0)) *
        video_width,
      height:
        ((json_data.normalized_bounding_box.bottom || 0) -
          (json_data.normalized_bounding_box.top || 0)) *
        video_height,
    };
  }
}
...
class Logo_Track {
  constructor(json_data, video_height, video_width) {
    this.start_time = nullable_time_offset_to_seconds(
      json_data.segment.start_time_offset
    );
    this.end_time = nullable_time_offset_to_seconds(
      json_data.segment.end_time_offset
    );
